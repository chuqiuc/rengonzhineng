...
In the first stage, it adopts DCP to restore visibility of the input hazy imag****e. 
...
2. Run the following command from `<RefineDNet_root>`.
```
python test_BeDDE.py --dataroot /media/jnu/BeDDE/ --dataset_mode simple_bedde --bedde_list /media/jnu/bedde_list.txt --name C3 --model refined_DCP --phase test --preprocess none --save_image --method_name C3_3 --epoch 3  --gpu_ids 0
``` 
...
```
python train.py --dataroot /mnt/RESIDE-unpaired --dataset_mode unpaired --model refined_DCP --name refined_DCP_outdoor --niter 30 --niter_decay 60 --lr_decay_iters 10 --preprocess scale_min_and_crop --load_size 300 --crop_size 256 --num_threads 8 --save_epoch_freq 3 
```
...
```
python train.py --name C4
```
nohup python train.py --name C5 --gpu_ids 0 --continue_train --epoch 15 --epoch_count 16 > C5_2.log 2>&1 &
nohup python train.py --name C5 --gpu_ids 0 > C5.log 2>&1 &
