...
from util1.convmod import TBlock
from util1.dice import DICE
from util1.mixdehaze import BasicLayer,SKFusion
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = ResLikeGenerator(input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout, r=10, eps=1e-3)
    else:
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif netD == 'TDiscriminator':
        net = TDiscriminator(input_nc,5)
    else:
...
class ResLikeGenerator(nn.Module):
    """Create a Unet-based generator"""

    def __init__(self, input_nc, output_nc, num_downs=5, ngf=6, norm_layer=nn.BatchNorm2d, use_dropout=False, r=10, eps=1e-3):
        """Construct a Unet generator
        Parameters:
            input_nc (int)  -- the number of channels in input images
            output_nc (int) -- the number of channels in output images
        ...
        It is a recursive process.
        """
        super(ResLikeGenerator, self).__init__()
        # construct unet structure
        self.model = ResnetGenerator(input_nc=input_nc, output_nc=output_nc, ngf=ngf, norm_layer=norm_layer, use_dropout=False, n_blocks=9, padding_type='reflect')
...
class efficientb1generater(nn.Module):
    def __init__(self,name: str, dataset: str, pretrained=True, weight_url: Optional[str] = None):
        super(efficientb1generater, self).__init__()
        self.model = create_seg_model(name,dataset,pretrained,weight_url)
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        self.conv3_19 = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=7, padding=9, groups=dim, dilation=3, padding_mode='reflect'),
            nn.BatchNorm2d(dim),
            nn.ReLU()
            )
        self.conv3_13 = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=5, padding=6, groups=dim, dilation=3, padding_mode='reflect'),
            nn.BatchNorm2d(dim),
            nn.ReLU()
            )
        self.conv3_7 = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=3, padding=3, groups=dim, dilation=3, padding_mode='reflect'),
            nn.BatchNorm2d(dim),
            nn.ReLU()
            )
        self.SEBlock = SE(dim,4)

        self.fuse = SKFusion(dim=dim,reduction=1)
    ...
    def forward(self, input):
        ...
        x = self.conv1(input)
        x = self.conv2(input)
        x = self.fuse([self.conv3_19(x)+self.conv3_13(x)+self.conv3_7(x),self.SEBlock(x)]) + residual
        return self.model(x)
    
class TDiscriminator(nn.Module):
    def __init__(self, input_nc,ngf,n_layers=5):
        super(TDiscriminator, self).__init__()
        sequence = [nn.Conv2d(input_nc, ngf, kernel_size=3, stride=1, padding=1), nn.LeakyReLU(0.2, True)]
        for i in range(n_layers):
            sequence+= [TBlock(ngf)]
        sequence += [nn.Conv2d(ngf , input_nc, kernel_size=3, stride=1, padding=1),
                    nn.LeakyReLU(0.2, True)]  # output 1 channel prediction map
        sequence += [nn.Conv2d(input_nc, 1, kernel_size=3, stride=1, padding=1)]
        self.model = nn.Sequential(*sequence)
    def forward(self, input):
        """Standard forward."""
        return self.model(input)
