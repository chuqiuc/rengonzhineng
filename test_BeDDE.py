...
if __name__ == '__main__':
    ...
    for i, data in enumerate(dataset):
        ...
        real_I = util.tensor2im(data['haze'], np.float64) # [0, 255], np.float
        real_J = util.tensor2im(data['clear'], np.float64) # [0, 255], np.float

        rec_J = util.tensor2im(visuals['rec_J'], np.float64) # [0, 255], np.float
        refine_J = util.tensor2im(visuals['refine_J'], np.float64) # [0, 255], np.float
